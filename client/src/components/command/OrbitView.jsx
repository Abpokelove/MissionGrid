...
const OrbitView = ({ objectives = [], compact = false, onSelectObjective }) => {
  ...
  const size = compact ? 350 : 460;
  const center = size / 2;
  const ringRadii = compact ? [66, 104, 142, 170] : [84, 132, 178, 220];

  return (
    <div className={`relative flex items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-space-950/35 ${compact ? 'min-h-[350px]' : 'min-h-[500px]'}`}>
      ...
      <div className="relative max-w-full" style={{ width: size, height: size }}>
        <img
          ...
          alt=""
          aria-hidden="true"
          className="pointer-events-none absolute left-1/2 top-1/2 h-[88%] w-[88%] -translate-x-1/2 -translate-y-1/2 rounded-full object-cover opacity-[0.22] mix-blend-screen drop-shadow-[0_0_70px_rgba(14,165,233,0.22)]"
          style={{
            WebkitMaskImage: 'radial-gradient(circle, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.64) 45%, rgba(0,0,0,0.22) 62%, transparent 76%)',
            maskImage: 'radial-gradient(circle, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.64) 45%, rgba(0,0,0,0.22) 62%, transparent 76%)',
          }}
        />
